...
open class LinearGradientLayer: CALayer, CAAnimationDelegate {
    ...
    open func actualRatiofromCircumference(_ ratio: CGFloat) -> CGFloat {
        if ratio > 1.0 {
            print("1.0")
    ...
    open func layerWithWithBounds(bounds:CGRect, position:CGPoint, colors: ColorGradient, linewidth : CGFloat,toValue:CGFloat) -> LinearGradientLayer {
        let layer = LinearGradientLayer(bounds: bounds, position: position, colors: colors)
        return layer
    }
    
    open func graint(fromColor:UIColor, toColor:UIColor, count:Int) -> [UIColor] {
    ...
    open func positionArrayWith(bounds:CGRect) -> [CGPoint]{
    ...
    open func startPoints() -> [CGPoint] {
        return [CGPoint.zero,CGPoint(x:1,y:0),CGPoint(x:1,y:1),CGPoint(x:0,y:1)]
    }
    
    open func endPoints() -> [CGPoint] {
        return [CGPoint(x:1,y:1),CGPoint(x:0,y:1),CGPoint.zero,CGPoint(x:1,y:0)]
    }
    
    open func midColorWithFromColor(fromColor:UIColor, toColor:UIColor, progress:CGFloat) -> UIColor {
        var fromR:CGFloat = 0.0,fromG:CGFloat = 0.0,fromB:CGFloat = 0.0,fromAlpha:CGFloat = 0.0
        fromColor.getRed(&fromR,green: &fromG,blue: &fromB,alpha: &fromAlpha)
        var toR:CGFloat = 0.0,toG:CGFloat = 0.0,toB:CGFloat = 0.0,toAlpha:CGFloat = 0.0
        toColor.getRed(&toR,green: &toG,blue: &toB,alpha: &toAlpha)
    ...
    open func animateCircle(duration: TimeInterval) {
    ...
    open func animateCircleTo(duration: TimeInterval, toValue: CGFloat) {
